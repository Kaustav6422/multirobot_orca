...
void formation_control::follower1_update()
{
    ...
    goal_x_robot1 = robot0_x + SWARM_DIST*cos(FORMATION_ANG) ;
    goal_y_robot1 = robot0_y + SWARM_DIST*sin(FORMATION_ANG) ;
    ...
    if (abs(robot0_vx) > 0.01) 
    {
        ...
        vel_msg_f1.linear.x  = min(robot0_vx*cos(e3_robot1) + k1*e1_robot1, MAX_LINEAR_VEL)        ; 
        vel_msg_f1.angular.z = min(robot0_omega + k2*robot0_vx*e2_robot1 + k3*robot0_vx*sin(e3_robot1), MAX_ANGULAR_VEL) ; 
    }
    else 
    {
...
void formation_control::follower2_update()
{
    ...
    goal_x_robot2 = robot0_x + SWARM_DIST*cos(FORMATION_ANG) ;
    goal_y_robot2 = robot0_y + SWARM_DIST*sin(FORMATION_ANG) ;
    ...
    if (abs(robot0_vx) > 0.01) 
    {
        ...
        vel_msg_f2.linear.x  = min(robot0_vx*cos(e3_robot2) + k1*e1_robot2, MAX_LINEAR_VEL)        ; 
        vel_msg_f2.angular.z = min(robot0_omega + k2*robot0_vx*e2_robot2 + k3*robot0_vx*sin(e3_robot2), MAX_ANGULAR_VEL) ; 
    }
    else 
    {
        ROS_INFO_STREAM("Controller TYPE TWO");
        delta_l_robot2 = sqrt(pow(goal_y_robot2-robot2_y,2) + pow(goal_x_robot2-robot2_x,2))   ;
        theta_robot2 = atan2(goal_y_robot2-robot2_y,goal_x_robot2-robot2_x)                    ;
...
void formation_control::update()
{
	...
	if ( now > t_next) 
	{
        ...
        debug_msg.angular.z = goal_y_robot1                   ;
